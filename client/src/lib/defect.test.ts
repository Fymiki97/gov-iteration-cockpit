...
const SAMPLE_DEFECTS: DefectRow[] = [
  { id: "d1", bugId: "BUG-1001", title: "登录页验证码刷新按钮点击无响应", priority: "最高", severity: "S-致命", status: "待处理", team: "政务AI", iteration: "V2.4", module: "用户认证", owner: "张明", reporter: "赵强", createdAt: "2026-09-10 09:23", deadline: "2026-09-15" },
  { id: "d3", bugId: "BUG-1003", title: "审批流流程节点配置后不生效", priority: "最高", severity: "S-致命", status: "处理中", team: "政务协作", iteration: "V2.4", module: "工作流引擎", owner: "王芳", reporter: "陈杰", createdAt: "2026-09-08 11:30", deadline: "2026-09-14" },
  { id: "d4", bugId: "BUG-1004", title: "短信验证码重复发送导致限流", priority: "较高", severity: "A-严重", status: "待处理", team: "政务AI", iteration: "V2.5", module: "用户认证", owner: "张明", reporter: "赵强", createdAt: "2026-09-11 10:02", deadline: "2026-09-17" },
  { id: "d6", bugId: "BUG-1006", title: "并行审批时会签节点丢失意见", priority: "最高", severity: "S-致命", status: "待处理", team: "政务协作", iteration: "V2.4", module: "工作流引擎", owner: "王芳", reporter: "陈杰", createdAt: "2026-09-07 09:18", deadline: "2026-09-13" },
  { id: "d7", bugId: "BUG-1007", title: "站内信未读红点数量不刷新", priority: "普通", severity: "B-一般", status: "待处理", team: "政务协作", iteration: "V2.5", module: "消息中心", owner: "刘洋", reporter: "孙倩", createdAt: "2026-09-13 08:55", deadline: "2026-09-22" },
  { id: "d13", bugId: "BUG-1013", title: "工作流超时提醒未触发", priority: "较高", severity: "A-严重", status: "已修复", team: "政务协作", iteration: "V2.3", module: "工作流引擎", owner: "王芳", reporter: "陈杰", createdAt: "2026-09-03 17:28", deadline: "2026-09-11" },
  { id: "d14", bugId: "BUG-1014", title: "导出记录筛选日期跨月查询为空", priority: "最低", severity: "C-低", status: "已修复", team: "WPS政务365", iteration: "V2.3", module: "数据导出", owner: "李华", reporter: "周敏", createdAt: "2026-09-02 09:36", deadline: "2026-09-10" },
];
...
describe("filterDefects", () => {
  it("searches by bug id or title", () => {
    const rows = filterDefects({ defects: SAMPLE_DEFECTS, search: "BUG-1001" });
    expect(rows.map((item) => item.bugId)).toEqual(["BUG-1001"]);
  });
  ...
  it("filters by team", () => {
    ...
    expect(rows.map((item) => item.bugId).sort()).toEqual([
      "BUG-1003", "BUG-1006", "BUG-1007", "BUG-1013",
    ]);
  });
  ...
  it("filters by iteration and keeps defects without iteration only when unfiltered", () => {
    ...
    expect(v24.map((item) => item.bugId).sort()).toEqual(["BUG-1001", "BUG-1003", "BUG-1006"]);

    const blank = { id: "d9", bugId: "BUG-1009", title: "无迭代缺陷", priority: "普通", severity: "B-一般", status: "待处理", team: "政务AI", iteration: "", module: "用户认证", owner: "张明", reporter: "赵强", createdAt: "2026-09-12 10:00", deadline: "" } as DefectRow;
  ...
  it("searches within iteration field", () => {
    ...
    expect(rows.map((item) => item.bugId).sort()).toEqual(["BUG-1013", "BUG-1014"]);
  });
});

describe("isOverdue", () => {
  it("marks unrepaired defects past deadline", () => {
    const overdue = SAMPLE_DEFECTS.find((item) => item.bugId === "BUG-1006");
    expect(overdue).toBeTruthy();
    expect(isOverdue(overdue!, now)).toBe(true);
  });

  it("does not mark resolved defects as overdue", () => {
    const resolved = SAMPLE_DEFECTS.find((item) => item.bugId === "BUG-1014");
...
describe("matchTaskDefects", () => {
  it("keeps unrepaired defects in the team and severity set", () => {
    ...
    expect(rows.map((item) => item.bugId).sort()).toEqual(["BUG-1001", "BUG-1004"]);
  });
});
...
describe("formatRemindMessage", () => {
  it("renders a preview with counts and defect lines", () => {
    ...
    expect(text).toContain("BUG-1001");
...
describe("nextBugId", () => {
  it("starts from BUG-1001 when the list is empty", () => {
    expect(nextBugId([])).toBe("BUG-1001");
  });
});
